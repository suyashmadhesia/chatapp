package com.example.Inbox.extension.plugin;
...
import com.example.Inbox.extension.embeded.RichTextInputChannel;
...
public class RichTextInputPlugin extends TextInputPlugin {
    @NonNull private final View mView;
    @NonNull private final InputMethodManager mImm;
    @NonNull private final AutofillManager afm;
    @NonNull private final RichTextInputChannel textInputChannel;
    @NonNull private InputTarget inputTarget = new InputTarget(InputTarget.Type.NO_TARGET, 0);
    @Nullable private RichTextInputChannel.Configuration configuration;
    @Nullable private SparseArray<RichTextInputChannel.Configuration> mAutofillConfigurations;
    ...
    public RichTextInputPlugin(
            View view,
            @NonNull RichTextInputChannel textInputChannel,
            @NonNull PlatformViewsController platformViewsController) {
        super(view, textInputChannel, platformViewsController);
        mView = view;
        ...
        textInputChannel.setTextInputMethodHandler(
                new RichTextInputChannel.TextInputMethodHandler() {
                    @Override
                    public void show() {
                        showTextInput(mView);
                    }
                    ...
                    public void setClient(
                            int textInputClientId, RichTextInputChannel.Configuration configuration) {
                    ...
                    @Override
                    public void setEditingState(RichTextInputChannel.TextEditState editingState) {
    ...
    public void destroy() {
        ...
        textInputChannel.setTextInputMethodHandler((RichTextInputChannel.TextInputMethodHandler) null);
    }



    private static int inputTypeFromTextInputType(
            RichTextInputChannel.InputType type,
            boolean obscureText,
            boolean autocorrect,
            boolean enableSuggestions,
            RichTextInputChannel.TextCapitalization textCapitalization) {
        if (type.type == RichTextInputChannel.TextInputType.DATETIME) {
            return InputType.TYPE_CLASS_DATETIME;
        } else if (type.type == RichTextInputChannel.TextInputType.NUMBER) {
            ...
            return textType;
        } else if (type.type == RichTextInputChannel.TextInputType.PHONE) {
            return InputType.TYPE_CLASS_PHONE;
        }

        int textType = InputType.TYPE_CLASS_TEXT;
        if (type.type == RichTextInputChannel.TextInputType.MULTILINE) {
            textType |= InputType.TYPE_TEXT_FLAG_MULTI_LINE;
        } else if (type.type == RichTextInputChannel.TextInputType.EMAIL_ADDRESS) {
            textType |= InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS;
        } else if (type.type == RichTextInputChannel.TextInputType.URL) {
            textType |= InputType.TYPE_TEXT_VARIATION_URI;
        } else if (type.type == RichTextInputChannel.TextInputType.VISIBLE_PASSWORD) {
            textType |= InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD;
        } else if (type.type == RichTextInputChannel.TextInputType.NAME) {
            textType |= InputType.TYPE_TEXT_VARIATION_PERSON_NAME;
        } else if (type.type == RichTextInputChannel.TextInputType.POSTAL_ADDRESS) {
        ...
        if (textCapitalization == RichTextInputChannel.TextCapitalization.CHARACTERS) {
            textType |= InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS;
        } else if (textCapitalization == RichTextInputChannel.TextCapitalization.WORDS) {
            textType |= InputType.TYPE_TEXT_FLAG_CAP_WORDS;
        } else if (textCapitalization == RichTextInputChannel.TextCapitalization.SENTENCES) {
    ...
    @VisibleForTesting
    void setTextInputClient(int client, RichTextInputChannel.Configuration configuration) {
    ...
    private void applyStateToSelection(RichTextInputChannel.TextEditState state) {
    ...
    @VisibleForTesting
    void setTextInputEditingState(View view, RichTextInputChannel.TextEditState state) {
    ...
    private void updateAutofillConfigurationIfNeeded(RichTextInputChannel.Configuration configuration) {
        notifyViewExited();
        this.configuration = configuration;
        final RichTextInputChannel.Configuration[] configurations = configuration.fields;
        ...
        if (configurations == null) {
            ...
            for (RichTextInputChannel.Configuration config : configurations) {
                RichTextInputChannel.Configuration.Autofill autofill = config.autofill;
                if (autofill == null) {
                    continue;
                }
    ...
    public void onProvideAutofillVirtualStructure(ViewStructure structure, int flags) {
        ...
        for (int i = 0; i < mAutofillConfigurations.size(); i++) {
            ...
            final RichTextInputChannel.Configuration config = mAutofillConfigurations.valueAt(i);
            final RichTextInputChannel.Configuration.Autofill autofill = config.autofill;
            if (autofill == null) {
                continue;
            }
    ...
    public void autofill(SparseArray<AutofillValue> values) {
        ...
        final RichTextInputChannel.Configuration.Autofill currentAutofill = configuration.autofill;
        ...
        for (int i = 0; i < values.size(); i++) {
            ...
            final RichTextInputChannel.Configuration config = mAutofillConfigurations.get(virtualId);
            ...
            final RichTextInputChannel.TextEditState newState =
                    new RichTextInputChannel.TextEditState(value, value.length(), value.length());
